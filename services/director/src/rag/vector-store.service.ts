...
export class VectorStoreService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    if (dbUrl) {
      ...
      this.store = await PGVectorStore.initialize(
        this.embedderService.getEmbeddings(),
        config
      );
    } else {
      this.isMemory = true;
  ...
  async similaritySearch(query: string, topK: number, projectId: string): Promise<Document[]> {
    return this.store.similaritySearch(query, topK, { projectId });
  }
}
