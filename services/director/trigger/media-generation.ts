...
export const generateImageTask = task({
  id: "generate-image",
  run: async (payload: { spaceId: string; stepId: string; prompt: string }) => {
    ...
    for (const part of response.candidates?.[0]?.content?.parts || []) {
      if (part.inlineData?.data) {
        ...
        const fileName = `generated/${payload.spaceId}/${payload.stepId}.png`;
...
export const generateVideoTask = task({
  id: "generate-video",
  run: async (payload: { spaceId: string; stepId: string; imageUrl: string; prompt: string }) => {
