...
import { Filter, GlProgram, Sprite, Texture, ImageSource, RenderTexture } from "pixi.js";
...
import type { GLTransition, TransitionOptions, TransitionRendererOptions } from "./types";
import {
...
import { PIXELATE_FRAGMENT, PIXELATE_UNIFORMS } from "../effect/glsl/custom-glsl";

export function makeTransition({ name, renderer }: TransitionOptions) {
  let transition: GLTransition | undefined = transitions.find((t: GLTransition) => t.name === name);

  if (!transition) {
    const allLocal = getAllTransitions();
    const localKey = Object.keys(allLocal).find((key) => key.toLowerCase() === name.toLowerCase());
    if (localKey) {
  ...
  if (!transition) {
    transition = transitions.find((t: GLTransition) => t.name.toLowerCase() === name.toLowerCase());
  }
  ...
  const sourceDisplacement = isDisplacementTransition ? new ImageSource({}) : undefined;
  ...
  const transitionLuma =
    transition.name === "luma" || name.toLowerCase() === "luma" || transition.label === "luma";
  ...
  const transitionHeart =
    transition.name === "heart" || name.toLowerCase() === "heart" || transition.label === "heart";
  ...
  return {
    render({ width, height, from, to, progress }: TransitionRendererOptions) {
      ...
      if (isDisplacementTransition && sourceDisplacement && defaultDisplacementTexture) {
        if (
