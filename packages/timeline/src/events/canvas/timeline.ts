...
function addNewTrack(this: Timeline, payload: IDropInfo) {
  ...
  const nextTrackIndex = (trackIndex === -1 ? this.tracks.length : trackIndex) as number;

  const [primaryTrackId] = Object.keys(primaryTracks);
  const currentTrack = this.tracks.find((track) => track.id === primaryTrackId)!;
  ...
  Object.keys(secondaryTracks).forEach((trackId) => {
    ...
    const currentSecondaryTrack = this.tracks.find((track) => track.id === trackId);

    if (!currentSecondaryTrack) return;

    const ids = objects.filter((obj) => obj.type !== "transition").map((obj) => obj.id);
...
function onTrackItemsMoved(this: Timeline, payload: IDropInfo) {
  const { isSecondaryOverlapped, secondaryTracks, primaryTracks, primaryPositions } = payload;
...
function onTrackItemsResized(
  ...
  if (!transition) {
    const currentTrack = this.tracks.find((track) => track.clipIds.includes(trackItemId))!;
    ...
    if (isOverlapped) {
      ...
      const currentTrackIndex = this.tracks.findIndex((track) => track.id === currentTrack.id);
  ...
  if (relationItemTransition) {
    const item = this.itemsManager.getTrackItems().find((obj) => obj.id === trackItemId)!;
    ...
    if (itemWidth <= firstTransitionWidth / 2 + secondTransitionWidth / 2) {
      this.transitionIds = this.transitionIds.filter((id) => id !== secondTransition?.id);
      const newTransitionMap: Record<string, ITransitionClip> = {};
      Object.keys(this.transitionsMap).forEach((id) => {
...
function onMouseUpClick(this: Timeline, e: TPointerEventInfo<TPointerEvent>) {
  ...
  if ((scenePoint.x === point.x || scenePoint.y === point.y) && !e.target) {
    ...
    const clientX =
      (e.e as MouseEvent).clientX || (e.e as unknown as TouchEvent).touches?.[0]?.clientX;
