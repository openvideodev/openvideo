 ...
 */
export const changeTrimmableWidth: TransformActionHandler = (_, transform, x, y) => {
  ...
  if (
    ...
    if (fromRight) {
      ...
      const widthToTime = unitsToTimeUs(newWidth, target.tScale, target.playbackRate);
      ...
      if (nextLeft < 0) {
        const durationInUnits = timeUsToUnits(target.duration, target.tScale, target.playbackRate);
