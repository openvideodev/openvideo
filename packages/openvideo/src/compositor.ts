    ...
    for (const sprite of sortedSprites) {
      ...
      if (timestamp < sprite.display.from || sprite.expired) {
        ...
        if (hasVideoTrack && pixiApp != null) {
          ...
          if (renderer) {
            await renderer.updateFrame(null, timestamp - sprite.display.from);
          }
        }
        continue;
      }
      ...
      if (hasVideoTrack && pixiApp != null && clipsNormalContainer != null) {
        ...
        if (renderer != null) {
          if (video != null) {
            hasVideo = true;
            await renderer.updateFrame(video, relativeTime);
          } else {
            await renderer.updateFrame(null, relativeTime);
          }
