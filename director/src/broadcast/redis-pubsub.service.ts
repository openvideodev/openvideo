...
export class RedisPubSubService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    this.subClient.on("pmessage", async (pattern, channel, message) => {
      try {
        ...
        this.logger.log(
          `Received update for project ${projectId}: ${data.type} (Status: ${data.status})`,
        );
  ...
  private async applyTaskResult(
    projectId: string,
    result: { imageUrl?: string; videoUrl?: string; prompt?: string },
  ) {
    ...
    let objectFit = clipPayload.objectFit;
    if (!objectFit && (clipPayload.type === "Image" || clipPayload.type === "Video")) {
      objectFit = "contain";
    }

    const state = core.getSnapshot();
    try {
      this.logger.log(`Adding generated media clip in redis-pubsub with objectFit: ${objectFit}`);
