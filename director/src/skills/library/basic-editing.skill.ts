...
export class BasicEditingSkill implements EditingSkill {
  name = "basic-editing";
  description =
    "Learn how to add, modify, and delete basic clips (Text, Image, Video, Audio) and adjust their properties (opacity, rotation, etc) using low-level commands.";
  ...
  resolve(context: ProjectContext, params?: Record<string, any>): Command[] {
    ...
    throw new Error(
      'basic-editing is an instructional skill. Please use `type: "command"` steps directly as documented in the skill.',
    );
  }
}
