...
#### What to do instead for Text / Caption clips

If the user asks for a fade or smooth entry/exit on a `Text` or `Caption` clip, use `effect-editing` instead:

- Use a `fadeIn` effect clip on the text track to fade in at the start.
- Use a `fadeOut` effect clip on the text track to fade out at the end.
- See the `effect-editing` skill for full details and examples.

---

### 1. Adding a Transition

A transition clip is placed at the overlapping intersection of two adjacent **Video or Image** clips.
