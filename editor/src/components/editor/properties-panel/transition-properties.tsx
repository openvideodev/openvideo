...
export function TransitionProperties({ clip }: TransitionPropertiesProps) {
  ...
  const handleUpdate = async (updates: any) => {
    ...
    const newKey = updates.key ?? coreClip.transitionKey;
    ...
    if (newDuration !== undefined || updates.key !== undefined) {
      ...
      const transitionStart = toClip.timing.display.from - newDuration / 2;
      ...
      const clipUpdates: any = {
        duration: newDuration,
        timing: { display: { from: Math.max(0, transitionStart), to: transitionEnd } },
      };

      if (updates.key) {
        clipUpdates.transitionKey = newKey;
      }
