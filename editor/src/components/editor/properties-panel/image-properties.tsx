    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          <InputGroup>
            ...
            <NumberInput
              value={Math.round(coreClip.transform?.x ?? coreClip.left ?? 0)}
              onChange={(val) => handleUpdate({ transform: { x: val } })}
              className="p-0"
            />
          </InputGroup>
          <InputGroup>
            ...
            <NumberInput
              value={Math.round(coreClip.transform?.y ?? coreClip.top ?? 0)}
              onChange={(val) => handleUpdate({ transform: { y: val } })}
              className="p-0"
            />
          </InputGroup>
        </div>
        <div className="grid grid-cols-2 gap-2">
          <InputGroup>
            ...
            <NumberInput
              value={Math.round(coreClip.transform?.width ?? coreClip.width ?? 0)}
              onChange={(val) => handleUpdate({ transform: { width: val } })}
              className="p-0"
            />
          </InputGroup>
          <InputGroup>
            ...
            <NumberInput
              value={Math.round(coreClip.transform?.height ?? coreClip.height ?? 0)}
              onChange={(val) => handleUpdate({ transform: { height: val } })}
              className="p-0"
            />
          </InputGroup>
        </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-4 mt-1">
          ...
          <Slider
            value={[Math.round(coreClip.transform?.angle ?? coreClip.angle ?? 0)]}
            onValueChange={(v) => handleUpdate({ transform: { angle: v[0] } })}
            max={360}
            step={1}
            className="flex-1"
          />
          <InputGroup className="w-20">
            <NumberInput
              value={Math.round(coreClip.transform?.angle ?? coreClip.angle ?? 0)}
              onChange={(val) => handleUpdate({ transform: { angle: val } })}
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-2 mt-1">
          <button
            onClick={() => {
              const flip = coreClip.transform?.flip ?? coreClip.flip ?? { x: false, y: false };
              handleUpdate({
                transform: {
                  flip: {
                    ...flip,
            ...
            className={`flex items-center justify-center flex-1 py-1.5 rounded-md border transition-colors ${
              (coreClip.transform?.flip?.x ?? coreClip.flip?.x)
          ...
          <button
            ...
            className={`flex items-center justify-center flex-1 py-1.5 rounded-md border transition-colors ${
              (coreClip.transform?.flip?.y ?? coreClip.flip?.y)
                ? "bg-primary/20 border-primary text-primary"
                : "bg-secondary/30 border-transparent text-muted-foreground hover:text-white"
            }`}
          >
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-4">
          ...
          <Slider
            value={[Math.round((coreClip.transform?.opacity ?? coreClip.opacity ?? 1) * 100)]}
            onValueChange={(v) => handleUpdate({ transform: { opacity: v[0] / 100 } })}
            max={100}
            step={1}
            className="flex-1"
          />
          <InputGroup className="w-20">
            <NumberInput
              value={Math.round((coreClip.transform?.opacity ?? coreClip.opacity ?? 1) * 100)}
              onChange={(val) => handleUpdate({ transform: { opacity: val / 100 } })}
              className="p-0 text-center"
            />
            <InputGroupAddon align="inline-end" className="p-0 pr-2">
              <span className="text-[10px] text-muted-foreground">%</span>
        ...
        <div className="grid grid-cols-2 gap-2">
          <InputGroup>
            ...
            <NumberInput
              ...
              onChange={(val) => handleShadowUpdate({ offsetX: val })}
              className="p-0"
            />
          </InputGroup>
          ...
          <InputGroup>
            ...
            <NumberInput
              ...
              onChange={(val) => handleShadowUpdate({ offsetY: val })}
              className="p-0"
            />
          </InputGroup>
        </div>
