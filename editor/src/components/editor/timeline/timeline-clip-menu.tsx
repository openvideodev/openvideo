...
export function TimelineClipMenu({ timelineCanvas }: TimelineClipMenuProps) {
  ...
  const activeClipId =
    isDropdownOpen || isHoveringMenu ? menuPosition?.clipId : hoveredClipId;

  const activeClip = activeClipId
    ? studio?.timeline.getClipById(activeClipId)
    : null;
  ...
  const isSelected =
    activeClip && selectedClips.some((c) => c.id === activeClip.id);

  if (
    !menuPosition ||
    !activeClip ||
    !isSelected ||
    activeClip.type === "Transition"
  )
    return null;
        ...
        <DropdownMenuTrigger asChild>
          <button
            ...
            onClick={(e) => {
              ...
              if (
                studio &&
                !selectedClips.find((c) => c.id === activeClip.id)
              ) {
