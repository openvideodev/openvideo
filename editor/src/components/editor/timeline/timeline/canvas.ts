      ...
      for (let i = 0; i < clipsAtTrack.length; i++) {
        ...
        if (x >= startX && x <= endX) {
          ...
          if (x < midpoint) {
            ...
            if (leftNeighbor) {
              const junctionX = (startX + (leftNeighbor.display.to / MICROSECONDS_PER_SECOND) * TIMELINE_CONSTANTS.PIXELS_PER_SECOND * this.#timeScale) / 2;
              if (this.isValidJunction(leftNeighbor, clip, junctionX, trackData, options)) {
                return { x: junctionX, clipA: leftNeighbor, clipB: clip, trackId: track.id, track };
              }
            } else if (rightNeighbor) {
              // Only right neighbor exists, snap to the only available junction
              const junctionX = (endX + (rightNeighbor.display.from / MICROSECONDS_PER_SECOND) * TIMELINE_CONSTANTS.PIXELS_PER_SECOND * this.#timeScale) / 2;
              if (this.isValidJunction(clip, rightNeighbor, junctionX, trackData, options)) {
                return { x: junctionX, clipA: clip, clipB: rightNeighbor, trackId: track.id, track };
              }
            }
          } else {
      ...
      trackData.clipIds.forEach((clipId) => {
        ...
        if (
          ...
          if (!timelineClip) {
            ...
            // In theory, a new clip won't be in a group, but we maintain consistency
            // with the update block below.
            ...
            if (timelineClip.group) {
              ...
              props.top =
                region.top - (group.top || 0) - (group.height || 0) / 2;
            }
